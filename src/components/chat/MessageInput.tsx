...
export function MessageInput({ channelId, onSend, disabled, placeholder, members, onTyping }: Props) {
      ...
      {aiSuggestions.length > 0 && (
        <div className="mb-2 space-y-1">
          ...
          {aiSuggestions.map((s, i) => (
            <button
              key={i}
              className="w-full rounded-md border bg-muted/30 px-3 py-2 text-left text-sm hover:bg-muted/60 transition-colors"
  ...
  const toolsPopover = (
    <Popover open={toolsOpen} onOpenChange={setToolsOpen}>
      <PopoverTrigger asChild>
        <button
          title="ツール"
          className="flex h-9 w-9 items-center justify-center rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground transition-colors"
        >
  ...
  // ========== モバイル: Voice-First レイアウト ==========
  if (isMobile) {
    return (
      <div className="relative shrink-0 border-t bg-background safe-area-bottom">
        {sharedOverlays}
        ...
        <div className="flex items-center gap-1 px-3 pt-2 pb-1">
          ...
          <button
            ...
            className="flex h-9 w-9 items-center justify-center rounded-lg hover:bg-muted text-muted-foreground hover:text-foreground disabled:opacity-50 transition-colors"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {/* テキストエリア（3行分確保） */}
        <div className="px-3">
          <div className="rounded-xl border bg-muted/30 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/40 transition-all">
            <Textarea
              ref={textareaRef}
              value={content}
              ...
              className="min-h-[80px] max-h-[160px] resize-none border-0 bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 text-[15px] leading-relaxed px-3 py-2.5"
              rows={3}
              disabled={disabled || uploading}
            />
          </div>
        ...
        <div className="flex items-center gap-2 px-3 py-2.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 h-10 px-3.5 rounded-xl bg-violet-50 dark:bg-violet-950/30 text-violet-600 dark:text-violet-400 hover:bg-violet-100 dark:hover:bg-violet-950/50 disabled:opacity-50 transition-colors text-sm font-medium"
          >
            {loadingSuggestions ? (

      ...
      <div className="p-4 space-y-6 max-w-4xl">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatCard
            ...
            mobile={isMobile}
            icon={
              <svg xmlns="http://www.w3.org/2000/svg" width={isMobile ? "22" : "18"} height={isMobile ? "22" : "18"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <StatCard
            ...
            icon={
              <svg xmlns="http://www.w3.org/2000/svg" width={isMobile ? "22" : "18"} height={isMobile ? "22" : "18"} viewBox="0 0 24 24" fill="none" stroke={stats.overdueTasks > 0 ? "#ef4444" : "currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <StatCard
            ...
            icon={
              <svg xmlns="http://www.w3.org/2000/svg" width={isMobile ? "22" : "18"} height={isMobile ? "22" : "18"} viewBox="0 0 24 24" fill="none" stroke={stats.dueTodayTasks > 0 ? "#f97316" : "currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          <StatCard
            ...
            icon={
              <svg xmlns="http://www.w3.org/2000/svg" width={isMobile ? "22" : "18"} height={isMobile ? "22" : "18"} viewBox="0 0 24 24" fill="none" stroke={stats.completedTodayTasks > 0 ? "#3b82f6" : "currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div>
            ...
            {upcomingTasks.length === 0 ? (
              <p className={cn("text-muted-foreground py-4", isMobile ? "text-[15px]" : "text-sm")}>期限が近いタスクはありません</p>
            ) : (
              <div className="space-y-2">
                {upcomingTasks.map((task, i) => (
                  <button
                    key={task.id}
                    className={cn(
                      "flex w-full items-center gap-3 rounded-lg border px-3 text-left hover:bg-muted/50 hover:shadow-sm transition-all duration-200 stagger-item",
                  ...
                  >
                    ...
                    {task.dueDate && (
                      <span className={cn("text-muted-foreground shrink-0 tabular-nums", isMobile ? "text-sm" : "text-xs")}>
          ...
          <div>
            ...
            {recentCompletedTasks.length === 0 ? (
              <p className={cn("text-muted-foreground py-4", isMobile ? "text-[15px]" : "text-sm")}>最近完了したタスクはありません</p>
            ) : (
              <div className="space-y-2">
                {recentCompletedTasks.map((task, i) => (
                  <div
                    key={task.id}
                    className={cn(
                      "flex items-center gap-3 rounded-lg border px-3 stagger-item",
                      isMobile ? "py-3.5" : "py-2.5"
                    )}
                    style={{ animationDelay: `${i * 50}ms` }}
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width={isMobile ? "18" : "16"} height={isMobile ? "18" : "16"} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-primary shrink-0">
                      <polyline points="20 6 9 17 4 12" />
                    </svg>
                    <div className="flex-1 min-w-0">
                      <div className={cn("truncate line-through text-muted-foreground", isMobile ? "text-[15px]" : "text-sm")}>
                        {task.title}
                      </div>
                    </div>
                    {task.assignee && (
                    ...
                    {task.completedAt && (
                      <span className={cn("text-muted-foreground shrink-0 tabular-nums", isMobile ? "text-xs" : "text-[10px]")}>

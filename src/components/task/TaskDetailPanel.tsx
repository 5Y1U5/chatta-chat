...
export function TaskDetailPanel({
  ...
  // コメントのリアルタイム購読（ユーザー情報はフック側で members から補完）
  useRealtimeComments({
      ...
      <div className="shrink-0 border-b px-4 py-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            <button
              className="flex md:hidden h-9 shrink-0 items-center gap-1 rounded-lg px-2 hover:bg-muted text-muted-foreground active:bg-muted/70 touch-manipulation transition-colors"
          ...
          <div className="flex items-center gap-1">
            {/* モバイル（md未満）: 完了ボタンを右側に配置（戻るボタンと離す） */}
            <Button
              size="sm"
              variant={currentTask.status === "done" ? "outline" : "default"}
              className={cn(

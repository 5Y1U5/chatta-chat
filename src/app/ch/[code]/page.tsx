...
export default function ChannelInvitePage({ params }: PageProps) {
  ...
  async function handleJoin() {
    setStatus("joining")
    try {
      const res = await fetch("/api/internal/channels/join", {
        method: "POST",
      ...
      const { workspaceId, channelId } = await res.json()
      setStatus("done")
      router.push(`/${workspaceId}/channel/${channelId}`)
      router.refresh()
    } catch {
              ...
              <>
                ...
                <div className="flex w-full gap-2">
                  <Button asChild className="flex-1">
                    <Link href={`/signup?chinvite=${code}`}>
                      新規登録
                    </Link>
                  </Button>
                  <Button asChild variant="outline" className="flex-1">
                    <Link href={`/login?chinvite=${code}`}>
                      ログイン
                    </Link>
                  </Button>
                </div>
